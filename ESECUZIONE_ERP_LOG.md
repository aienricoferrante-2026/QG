...
### ✅ FLIP COMMESSE ESEGUITO (29/06, "vai") — 5ª app, aggregatore multi-client
F0: commesse legge da 3 client — **stwClient** (STW `odjwvqab`, SOLA LETTURA: commesse 14k/discenti, fonte canonica) + **hubClient** (bqyqr) + **supabaseAdmin** (bhroni, le SUE 4 tabelle, scritture). Solo `supabaseAdmin` scrive → flippo solo quello (come le altre), lascio il read-only STW sul master (già fonte di consolidamento, dati identici a bqyqr.commesse).
- **app_commesse** = 4 tabelle bhroni migrate (commessa_filiale_map 928, filiale_override 1, utenti 1, audit_log 0). 0 funzioni/0 trigger (app semplice) + extensions nel search_path (lezione digest). Esposto + grant service_role. Letture REST OK.
- **3 env ERP_*** su `qualifica-wea-commesse` (supabaseAdmin→app_commesse; STW/HUB invariati) + **redeploy Ready**, app viva (302), 0 errori.
- **Rollback:** togliere 3 env ERP_* da qualifica-wea-commesse.
