...
/* ── Tree Table: expandable rows with sub-levels ──
   data: array of raw items
   config: {
     primaryField: 'agente',      -- group by this field
     primaryLabel: 'Commerciale',
     subGroupField: 'status',     -- sub-group by this field
     subGroupLabel: 'Status',
     valueFn: (items) => [{label, val}], -- columns for each group row
     subValueFn: (items) => [{label, val}], -- columns for each sub-group row
     itemColumns: [{hdr, fn: (item) => display}] -- columns for leaf items
   }
*/
function buildTreeTbl(id, data, config) {
  ...
  sorted.forEach(([key, items], gi) => {
    ...
    // Sub-group rows (hidden by default)
    if (config.subGroupField) {
      const subG = {};
      items.forEach(c => {
        const sk = c[config.subGroupField] || 'N/D';
        if (!subG[sk]) subG[sk] = [];
        subG[sk].push(c);
      });
      const subSorted = Object.entries(subG).sort((a, b) => b[1].length - a[1].length);

      subSorted.forEach(([sk, sItems], si) => {
        const sid = gid + '_s' + si;
        ...
        if (config.itemColumns) {
          sItems.forEach((c, ci) => {
            h += '<tr class="tree-row tree-l2 tree-child-' + sid + '" style="display:none">';
            h += '<td></td>';
            h += '<td style="padding-left:48px;font-size:10px">';
            config.itemColumns.forEach((col, coli) => {
              if (coli > 0) h += ' &middot; ';
              h += col.fn(c);
            });
            h += '</td>';
            const leafVals = config.leafValueFn ? config.leafValueFn(c) : [];
            leafVals.forEach(v => { h += '<td class="text-right" style="font-size:10px">' + v + '</td>'; });
            // Fill remaining cols
            for (let fi = leafVals.length; fi < cols.length; fi++) h += '<td></td>';
            h += '</tr>';
          });
        }
      });
...
function toggleTree(gid) {
  ...
  if (visible) {
    // Collapse: hide direct children AND their sub-children
    children.forEach(r => {
      r.style.display = 'none';
      // Also collapse any nested
