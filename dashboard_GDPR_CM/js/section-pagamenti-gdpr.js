...
function renderPagamentiGdpr() {
  ...
  /* 🚨 Anomalie GDPR: Insoluto anno precedente senza accordo */
  const anom = f.filter(c =>
    /Insoluto|Giallo.*Rosso/i.test(c.gdprStatoPag || '') &&
    !(c.gdprAccordo && String(c.gdprAccordo).trim())
  );
  if (anom.length) {
    const a = document.createElement('div');
    a.className = 'card';
    a.style.cssText = 'margin-top:14px;border-left:3px solid #dc2626';
    a.innerHTML = '<h4 style="color:#dc2626">🚨 Anomalia · Insoluti senza accordo pagamento (' + fmt(anom.length) + ')</h4>' +
      '<p style="color:var(--text3);font-size:11px;margin-bottom:10px">Pratiche GDPR con stato pagamento Giallo-Rosso ma senza accordo di rientro formalizzato.</p>' +
      '<div class="tbl-scroll"><table id="tblGdprAnomPag"></table></div>';
    el.appendChild(a);
    buildTbl('tblGdprAnomPag',
...
function _gdprRenderPivot() {
  if (typeof buildPivotCard !== 'function') return;
  buildPivotCard({
    containerId: 'sec-pagamentiGdpr',
    cardId: 'gdprPivotCard',
    stateNamespace: 'gdpr',
    title: '🌳 Pivot GDPR · esplora gerarchico',
    ...
    dims: {
      ...
      statoPag:  { label: 'Stato Pagamento',  extract: c => c.gdprStatoPag || 'N/D' },
      accordo:   { label: 'Accordo Pagamenti', extract: c => (c.gdprAccordo && String(c.gdprAccordo).trim()) ? 'Sì' : 'No' },
    ...
    presets: [
      { label: '🚦 Status → StatoPag',        dims: ['status', 'statoPag', '', ''] },
      { label: '💰 StatoPag → Accordo → Cliente', dims: ['statoPag', 'accordo', 'cliente', ''] },
      { label: '💼 Commerciale → Status',     dims: ['agente', 'status', '', ''] },
      { label: '⚙️ Tecnico → Status',          dims: ['responsabile', 'status', 'statoLav', ''] },
    ],
    defaultDims: ['status', 'statoPag', 'accordo', ''],
    items: filtered,
  });
}
