#!/usr/bin/env python3
# Allinea app_sales <- sorgente sulle tabelle con gap (trigger DISABILITATI durante
...
def q(ref,sql):
    for a in range(6):
        ...
        try:
            ...
            raise
TABLES=["ordine_cliente","opportunita","deal","anagrafica_cliente"]
for t in TABLES:
    ...
    q(HUB,f"alter table app_sales.{t} disable trigger all")
    ...
    try:
        while True:
            try:
                ...
                r=q(HUB,f"insert into app_sales.{t} ({sel}) select {sel} from jsonb_to_recordset($j${data}$j$::jsonb) as r({rec}) on conflict ({pk}) do nothing")
        ...
        q(HUB,f"alter table app_sales.{t} enable trigger all")
    cnt=q(HUB,f"select count(*) n from app_sales.{t}")[0]["n"]
